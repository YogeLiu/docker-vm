...
type RuntimeInstance interface {
	...
	Invoke(contractId *common.Contract, method string, byteCode []byte, parameters map[string][]byte,
		txContext TxSimContext, gasUsed uint64) *common.ContractResult
}

//WasmerInstancesManager used to generate a wasmer instance
type WasmerInstancesManager interface {
	// NewRuntimeInstance get a runtime instance
	NewRuntimeInstance(contract *common.Contract, byteCode []byte) (RuntimeInstance, error)
}
//
////NativeFactory used to generate a native instance
//type NativeFactory interface {
//	// NewRuntimeInstance get a runtime instance
//	NewRuntimeInstance(contract *common.Contract, byteCode []byte) (RuntimeInstance, error)
//}

//EvmInstancesManager used to generate a evm instance
type EvmInstancesManager interface {
	// NewRuntimeInstance get a runtime instance
	NewRuntimeInstance(txSimContext TxSimContext, chainId, method string, contract *common.Contract,
		byteCode []byte, log Logger) (RuntimeInstance, error)
}

//WxvmInstancesManager used to generate a wxvm instance
type WxvmInstancesManager interface {
	// NewRuntimeInstance get a runtime instance
	NewRuntimeInstance(chainId string, log Logger, byteCode []byte, codePath string) (RuntimeInstance, error)
}

//GasmInstancesManager used to generate a gasm instance
type GasmInstancesManager interface {
	// NewRuntimeInstance get a runtime instance
	NewRuntimeInstance(chainId string, log Logger, byteCode []byte) (RuntimeInstance, error)
}
